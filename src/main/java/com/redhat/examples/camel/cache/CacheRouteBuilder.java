		...
		from("cxfrs:bean:rsServer?bindingStyle=SimpleConsumer")
			...
			.choice()
				.when(header("operationName").isEqualTo("addEntry"))
					...
					.log("${headers}")
					.to("bean:refreshCacheBean?method=addEntry")
					.to("cache://MyApplicationCache")
					.to("bean:refreshCacheBean?method=addedEntry")
				.when(header("operationName").isEqualTo("getEntry"))
					.log("In get entry choice")
					.to("bean:refreshCacheBean?method=getEntry")
					.to("cache://MyApplicationCache")
					.log("${headers}")
					.choice()
						.when(header(CacheConstants.CACHE_ELEMENT_WAS_FOUND).isNull())
				        .to("bean:refreshCacheBean?method=notFound")
				        .otherwise()
				        .to("bean:refreshCacheBean?method=gotEntry")
				    .end()
				.end();
	}
}